...
class FormUpdateScreen extends StatefulWidget {
  ...
  const FormUpdateScreen({super.key, required this.horario});

  @override
  State<FormUpdateScreen> createState() => _FormUpdateScreenState();
}
...
class _FormUpdateScreenState extends State<FormUpdateScreen> {
  void _loadData() async {
    ...
    idProfesorController = widget.horario.profesorHorario.idProfesor;
    idMateriaController = widget.horario.materiaHorario.idMateria;
  ...
  void initState() {
    super.initState();
    _loadData();
    edificioController.text = widget.horario.edificioHorario;
    salonController.text = widget.horario.salonHorario;
    horaInicioController.text = widget.horario.horaHorario.split('-')[0];
    horaFinController.text = widget.horario.horaHorario.split('-')[1];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Actualizar nuevo horario'),
      ),
      body: _displayBody(),
    );
  }
              ...
              onTap: () {
                showTimePicker(
                  ...
                  initialTime: TimeOfDay(
                    ...
                    minute: int.parse(horaInicioController.text.split(':')[1].substring(0, 2)),
                  ),
                ).then((value) {
                  if (value != null) {
                    // guarda la hora en el controlador con formato de 24 horas
  ...
  void _updateOneHorario() {
    ...
    HorarioController.updateOne(horario).then((value) {
      _showMessage('Horario actualizado correctamente');
